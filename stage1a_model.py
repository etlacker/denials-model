"""
Denial prediction model trained on the cleaned/enriched dataset.
...
Usage:
  python stage1a_model.py                           # Uses augmented data if available
  python stage1a_model.py --data-dir artifacts/data_cleaning_raw  # Uses raw data
  python stage1a_model.py --both                    # Runs on both datasets
"""
import argparse
...
def main():
    parser = argparse.ArgumentParser(description="Train XGBoost denial prediction model.")
    parser.add_argument(
        "--data-dir",
        type=Path,
    ...
    if args.both:
        ...
        for suffix in ["raw", "augmented", "balanced"]:
            ...
            if not (data_dir / "claims_enriched_train.csv").exists():
                ...
                continue
            output_dir = ARTIFACTS_DIR / f"stage1a_{suffix}"
            print("\n" + "=" * 60)
        ...
        if args.output_dir:
            ...
            output_dir = ARTIFACTS_DIR / "stage1a_balanced"
        elif "raw" in str(data_dir):
            output_dir = ARTIFACTS_DIR / "stage1a_raw"
        elif "augmented" in str(data_dir):
            output_dir = ARTIFACTS_DIR / "stage1a_augmented"
        else:
            output_dir = ARTIFACTS_DIR / "stage1a"
